...
<% RobotConfig robotConfig = (RobotConfig) request.getAttribute("config");
    if (robotConfig == null) {
        response.sendRedirect(request.getContextPath() + "/config");
        return;
    }
    List<ComandoRobot> historial
    ...
    <body>
        ...
        <header class="topbar">
            ...
            <div class="topbar-tabs">
                ...
                <button class="tab-btn" onclick="cambiarTab('grabar', this)">📹 Grabar</button>
                <button class="tab-btn" onclick="cambiarTab('grabarManual', this)">✍🏻 Grabado Manual</button>
        ...
        <div class="panels">
            <div style="display:flex;flex-direction:column;overflow:hidden;">
                ...
                <!-- ========== TAB GRABAR ========== -->
                <div class="tab-panel" id="tab-grabar">
                    <div class="camera-wrap" id="cameraWrap">
                        <div class="cam-corner tl"></div>
                        <div class="cam-corner tr"></div>
                        <div class="cam-corner bl"></div>
                    ...
                    <div class="controls-bar">
                        <div class="extras-col">
                            <button class="btn-extra" onclick="enviarCmd('ledon')">💡 LED ON</button>
                            <button class="btn-extra" onclick="enviarCmd('ledoff')">🌑 LED OFF</button>
                            <button class="btn-extra" onclick="reloadCam()">📷 Cam</button>
                        </div>
                        <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">

                            <div style="display:flex; gap:8px; width:100%; justify-content:center;">
                                <button class="btn-extra" id="btnIniciar" onclick="iniciarGrabacion()">
                                    📹 Iniciar
                                </button>
                            ...
                            <!-- Panel D-PAD (Oculto por defecto mediante display:none) -->
                            <div class="dpad" id="panelDpad" style="display:none;">
                                <button class="btn-d d-up" id="bUp" onmousedown="startMove('go')"
                                        onmouseup="stopMove()" onmouseleave="stopMove()"
                                        ontouchstart="startMove('go')"
                                        ontouchend="stopMove()">↑<span>W/↑</span></button>
        ...
        <script>
            ...
            function enviarCmd(cmd, showToast = true) {
                ...
                return fetch(CTX + '/control', {method: 'POST', body: fd})
                        ...
                        .then(d => {
                            if (showToast)
                                toast(d.exito ? '✓' : '✕', d.mensaje, d.color);
                            addLog(d, cmd);
                            return d;
                        }).catch(() => {
                    if (showToast)
                        toast('✕', 'Error de red', '#f00');
                });
            }

            /* CÁMARA (Para evitar los ReferenceError) */
            function camaraOk() {
                if (document.getElementById('camOffline'))
            ...
            function reloadCam() {
                ...
                if (f)
                    f.src = CTX + '/camera-proxy?t=' + Date.now();
            }
            ...
            function startMove(c) {
                ...
                _mAct = c;
                enviarCmd(c);
                _mInt = setInterval(() => enviarCmd(c, false), 300);
            }
            function stopMove() {
                if (!_mAct)
                    return;
            ...
            function detenerAutonomo() {
                _aRun = false;
                clearTimeout(_aT);
                enviarCmd('stop');
                document.getElementById('btnIniciarAuto').style.display = 'inline-flex';
                document.getElementById('btnDetenerAuto').style.display = 'none';
                document.getElementById('autoStatusTxt').textContent = 'Listo';
            }

            function cambiarTab(t, b) {
            ...
            function updateSpeed(v) {
                document.getElementById('speedNum2').textContent = v;
                enviarCmd('speed-' + v, false);
            }
            ...
            function cargarFotos() {
                fetch(CTX + '/foto?accion=lista').then(r => r.json()).then(d => {
                    ...
                    if (!d.total) {
                        grid.innerHTML = '<div class="foto-empty">📷 No hay fotos</div>';
                        return;
                    }
            ...
            function verFoto(idx) {
                ...
                document.getElementById('btnEliminarIndividual').onclick = () => {
                    eliminarFoto(idx);
                };
            ...
            function limpiarFotos() {
                if (!confirm('¿Borrar galería?'))
                    return;
                fetch(CTX + '/foto?accion=limpiar').then(() => {
                    cargarFotos();
                    toast('🗑', 'Limpio', '#ffcc00');
                });
            }
            ...
            function activarG() {
                _gRun = true;
                document.getElementById('btnGyroStart').style.display = 'none';
                document.getElementById('btnGyroStop').style.display = 'block';
                window.addEventListener('deviceorientation', manejarG);
                _gInt = setInterval(() => {
                    if (_gCmd)
                        enviarCmd(_gCmd, false);
                }, 300);
            }
            function detenerGiro() {
                _gRun = false;
                window.removeEventListener('deviceorientation', manejarG);
                clearInterval(_gInt);
                _gCmd = null;
                enviarCmd('stop');
                document.getElementById('btnGyroStart').style.display = 'block';
                document.getElementById('btnGyroStop').style.display = 'none';
            }
            function manejarG(e) {
                ...
                if (y > 25)
                    ...
                    c = 'left';
                _gCmd = c;
                if (document.getElementById('gyroCmdTxt'))
                    document.getElementById('gyroCmdTxt').textContent = c ? c.toUpperCase() : 'NEUTRO';
            }
            ...
            if (document.getElementById('btnGyroStart'))
                document.getElementById('btnGyroStart').onclick = iniciarGiro;
            if (document.getElementById('btnGyroStop'))
                document.getElementById('btnGyroStop').onclick = detenerGiro;

            setInterval(() => {
                fetch(CTX + '/status').then(r => r.json()).then(d => {
                    document.getElementById('statusTxt').textContent = d.conectado ? 'ONLINE' : 'OFFLINE';
                    document.getElementById('statusDot').className = 'dot' + (d.conectado ? ' online' : '');
                }).catch(() => {
                });
            }, 5000);
        </script>
